...
public class AlohAndesTM {
	...
	//RFC 1
		...
		public List<VOMejorCliente> buenosClientes() throws Exception
		{
			DAOCliente dao=new DAOCliente();
			ArrayList<VOMejorCliente> clientes;
			ArrayList<VOMejorCliente> resp=new ArrayList<>();
			ArrayList<Integer> temp=new ArrayList<>();
			try 
			{
				this.conn=darConexion();
				dao.setConn(conn);
				clientes=dao.mejoresClientes();
				for(VOMejorCliente x:clientes)
				{
					if(temp.contains(x.getId()))
						continue;
					else 
					{
						temp.add(x.getId());
						ClienteContrato y=getClienteContratoById(x.getId());
						Date ahora=new Date(System.currentTimeMillis());
						Date inicio=Date.valueOf(y.getCliente().getFechaIngreso());
						if(inicio!=null) {
							int anios=ahora.getYear()-ahora.getYear();
							if(y.getContratos().size()>=(anios*12-ahora.getMonth()))
							{
								resp.add(x);
							}
						}
